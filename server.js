...
const adminExist = db.prepare('SELECT id FROM users WHERE username = ?').get('13135629285');
if (!adminExist) {
  const hash = bcrypt.hashSync('13135629285', 10);
  db.prepare("INSERT INTO users (username, password, role, status) VALUES (?, ?, 'admin', 'approved')").run('13135629285', hash);
}
...
// Auth
app.post('/api/register', idCardUpload.fields([{ name: 'id_front' }, { name: 'id_back' }]), (req, res) => {
  try {
    ...
    db.prepare("INSERT INTO users (username, password, role, status, id_card_front, id_card_back) VALUES (?, ?, 'user', 'pending', ?, ?)").run(username, hash, front, back);
...
app.get('/api/me', auth, (req, res) => {
  const u = db.prepare('SELECT id, username, role, status FROM users WHERE id = ?').get(req.userId);
  res.json(u);
});

// Client Stats
app.get('/api/stats', auth, (req, res) => {
  const all = db.prepare('SELECT COALESCE(SUM(actual_payment),0) as spend, COALESCE(SUM(selling_price),0) as sold, COALESCE(SUM(COALESCE(selling_price,0)-actual_payment),0) as profit, COUNT(*) as cnt FROM orders WHERE user_id=?').get(req.userId);
  ...
  const thisM = db.prepare("SELECT COALESCE(SUM(COALESCE(selling_price,0)-actual_payment),0) as profit FROM orders WHERE user_id=? AND order_status='已卖出' AND created_at>=? AND created_at<?").get(req.userId, ms, me);
  const lastM = db.prepare("SELECT COALESCE(SUM(COALESCE(selling_price,0)-actual_payment),0) as profit FROM orders WHERE user_id=? AND order_status='已卖出' AND created_at>=? AND created_at<?").get(req.userId, lms, ms);
...
app.get('/api/orders', auth, (req, res) => {
  ...
  const cnt = db.prepare('SELECT COUNT(*) as total FROM orders o '+w).get(...p);
  const list = db.prepare('SELECT o.* FROM orders o '+w+' ORDER BY o.'+sortBy+' '+sortOrder+' LIMIT ? OFFSET ?').all(...p, pageSize, (page-1)*pageSize);
...
app.post('/api/orders', auth, (req, res) => {
  ...
  if (!o.platform || !o.product_name || !o.tracking_no || o.purchase_price == null) return res.status(400).json({ error: '必填字段不完整' });
    ...
    const r = db.prepare('INSERT INTO orders (user_id,platform,product_name,product_spec,purchase_price,coupon_amount,actual_payment,selling_price,selling_platform,tracking_no,logistics_status,order_status,buyer_name,buyer_contact,settlement_status,note) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(
...
app.put('/api/orders/:id', auth, (req, res) => {
  ...
  try {
    ...
    db.prepare('UPDATE orders SET platform=?,product_name=?,product_spec=?,purchase_price=?,coupon_amount=?,actual_payment=?,selling_price=?,selling_platform=?,tracking_no=?,logistics_status=?,order_status=?,buyer_name=?,buyer_contact=?,settlement_status=?,note=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(
...
app.get('/api/orders/export', auth, (req, res) => {
...
app.get('/api/admin/stats', auth, adminOnly, (req, res) => {
  const g = db.prepare("SELECT COALESCE(SUM(actual_payment),0) as spend, COALESCE(SUM(selling_price),0) as sold, COALESCE(SUM(COALESCE(selling_price,0)-actual_payment),0) as profit, COUNT(*) as cnt, COUNT(DISTINCT user_id) as members FROM orders").get();
  const now = new Date(); const m = now.getMonth()+1; const y = now.getFullYear();
  const ms = y+'-'+String(m).padStart(2,'0')+'-01';
  const me = (m===12?(y+1):y)+'-'+String(m===12?1:m+1).padStart(2,'0')+'-01';
  const lm = m===1?12:m-1; const ly = m===1?y-1:y;
  const lms = ly+'-'+String(lm).padStart(2,'0')+'-01';
  const tm = db.prepare("SELECT COALESCE(SUM(COALESCE(selling_price,0)-actual_payment),0) as profit FROM orders WHERE order_status='已卖出' AND created_at>=? AND created_at<?").get(ms, me);
  const lm_ = db.prepare("SELECT COALESCE(SUM(COALESCE(selling_price,0)-actual_payment),0) as profit FROM orders WHERE order_status='已卖出' AND created_at>=? AND created_at<?").get(lms, ms);
  const members = db.prepare("SELECT u.id,u.username,u.role, (SELECT COUNT(*) FROM orders o WHERE o.user_id=u.id) as oCnt, COALESCE((SELECT SUM(o2.actual_payment) FROM orders o2 WHERE o2.user_id=u.id),0) as spend, COALESCE((SELECT SUM(o2.selling_price) FROM orders o2 WHERE o2.user_id=u.id),0) as sold, COALESCE((SELECT SUM(COALESCE(o2.selling_price,0)-o2.actual_payment) FROM orders o2 WHERE o2.user_id=u.id),0) as profit FROM users u WHERE u.status='approved' ORDER BY profit DESC").all();
...
app.get('/api/admin/orders', auth, adminOnly, (req, res) => {
  ...
  if (search) { w += ' AND (o.product_name LIKE ? OR o.tracking_no LIKE ?)'; p.push('%'+search+'%','%'+search+'%'); }
  const cnt = db.prepare('SELECT COUNT(*) as total FROM orders o '+w).get(...p);
  const list = db.prepare('SELECT o.*, u.username FROM orders o LEFT JOIN users u ON o.user_id=u.id '+w+' ORDER BY o.'+sortBy+' '+sortOrder+' LIMIT ? OFFSET ?').all(...p, pageSize, (page-1)*pageSize);
  res.json({ list, total: cnt.total, page, pageSize });
});

app.get('/api/admin/orders/:id', auth, adminOnly, (req, res) => {
  const order = db.prepare('SELECT o.*, u.username FROM orders o JOIN users u ON u.id=o.user_id WHERE o.id=?').get(req.params.id);
  if (!order) return res.status(404).json({ error: '订单不存在' });
  res.json(order);
});
...
app.get('/api/admin/export', auth, adminOnly, (req, res) => {
  const rows = db.prepare('SELECT o.*, u.username FROM orders o LEFT JOIN users u ON o.user_id=u.id ORDER BY o.created_at DESC').all();
...
app.get('/api/admin/users', auth, adminOnly, (req, res) => {
  const users = db.prepare("SELECT u.id,u.username,u.role,u.status,u.id_card_front,u.id_card_back,u.created_at,(SELECT COUNT(*) FROM orders o WHERE o.user_id=u.id) as oCnt, COALESCE((SELECT SUM(o2.actual_payment) FROM orders o2 WHERE o2.user_id=u.id),0) as spend, COALESCE((SELECT SUM(o2.selling_price) FROM orders o2 WHERE o2.user_id=u.id),0) as sold, COALESCE((SELECT SUM(COALESCE(o2.selling_price,0)-o2.actual_payment) FROM orders o2 WHERE o2.user_id=u.id),0) as profit FROM users u ORDER BY u.created_at DESC").all();
  res.json(users);
});
...
app.post('/api/admin/users', auth, adminOnly, (req, res) => {
  ...
  const hash = bcrypt.hashSync(password, 10);
  db.prepare("INSERT INTO users (username, password, role, status) VALUES (?,?,'user','approved')").run(username, hash);
...
app.get('/api/admin/reviews', auth, adminOnly, (req, res) => {
  res.json(db.prepare("SELECT * FROM users WHERE status IN ('pending','rejected') ORDER BY created_at DESC").all());
});
